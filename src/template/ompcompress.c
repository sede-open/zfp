...
_t2(compress_omp, Scalar, 1)(zfp_stream* stream, const zfp_field* field)
{
  ...
  for (chunk = 0; chunk < (int)chunks; chunk++) {
    ...
    for (block = bmin; block < bmax; block++) {
      ...
      if (nx - x < 4)
        if (length_table)
          length_table[block] = _t2(zfp_encode_partial_block_strided, Scalar, 1)(&s, p, nx - x, 1);
        else
...
_t2(compress_strided_omp, Scalar, 1)(zfp_stream* stream, const zfp_field* field)
{
  ...
  for (chunk = 0; chunk < (int)chunks; chunk++) {
    ...
    for (block = bmin; block < bmax; block++) {
      ...
      if (nx - x < 4u)
        if (length_table)
          length_table[block] = _t2(zfp_encode_partial_block_strided, Scalar, 1)(&s, p, nx - x, sx);
        else
